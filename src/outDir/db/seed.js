...
const Acao_1 = require("../classes/Acao");
const RendaFixa_1 = require("../classes/RendaFixa");
const FundoInvestimento_1 = require("../classes/FundoInvestimento");
...
const prisma = {
    ...
    user: repositories_1.usuarioRepository,
    investment: repositories_1.investimentoRepository,
    portfolio: repositories_1.portfolioRepository,
    favorite: repositories_1.favoritoRepository,
    notification: repositories_1.notificacaoRepository
};
async function main() {
    console.log('Iniciando seed do sistema de investimentos...');
    try {
        LoggingMiddleware_1.LoggingMiddleware.info('Iniciando seed do banco de dados');
        // Limpar dados existentes
        await clearDatabase();
        LoggingMiddleware_1.LoggingMiddleware.info('Banco de dados limpo');
        // Criar usuários
        ...
        LoggingMiddleware_1.LoggingMiddleware.error('Erro durante o seed do banco de dados', undefined, undefined, error instanceof Error ? error.message : String(error));
        throw error;
    }
}
async function clearDatabase() {
    await repositories_1.usuarioRepository.clear();
    await repositories_1.investimentoRepository.clear();
    await repositories_1.portfolioRepository.clear();
    await repositories_1.favoritoRepository.clear();
    await repositories_1.notificacaoRepository.clear();
}
async function createUser(userData) {
    const usuario = new Usuario_1.Usuario(userData.id, userData.nome, userData.email, userData.senha);
    await repositories_1.usuarioRepository.create(usuario);
    return usuario;
}
async function createAcao(acaoData) {
    const acao = new Acao_1.Acao(acaoData.id, acaoData.nome, acaoData.codigo, acaoData.historico, acaoData.volatilidade, acaoData.dividendYield);
    await repositories_1.investimentoRepository.create(acao);
    return acao;
}
async function createRendaFixa(rfData) {
    const rendaFixa = new RendaFixa_1.RendaFixa(rfData.id, rfData.nome, rfData.codigo, rfData.historico, rfData.taxaJuros, rfData.dataVencimento);
    await repositories_1.investimentoRepository.create(rendaFixa);
    return rendaFixa;
}
async function createFundo(fundoData) {
    const fundo = new FundoInvestimento_1.FundoInvestimento(fundoData.id, fundoData.nome, fundoData.codigo, fundoData.historico, fundoData.taxaAdministracao, fundoData.patrimonioTotal);
    await repositories_1.investimentoRepository.create(fundo);
    return fundo;
}
async function createPortfolio(portfolioData) {
    const portfolio = new Portfolio_1.Portfolio(portfolioData.id, portfolioData.nome, portfolioData.usuario, portfolioData.descricao);
    // Adicionar investimentos ao portfólio
    for (const item of portfolioData.investimentos) {
        portfolio.adicionarInvestimento(item.investimento, item.quantidade, item.precoCompra);
    }
    await repositories_1.portfolioRepository.create(portfolio);
    return portfolio;
}
async function createManyFavoritos(favoritosData) {
    for (const favData of favoritosData) {
        const dataAdicao = new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000);
        const favorito = new Favorito_1.Favorito(favData.usuario, favData.investimento, dataAdicao);
        await repositories_1.favoritoRepository.create(favorito);
    }
}
async function createManyNotificacoes(notificacoesData) {
    for (let i = 0; i < notificacoesData.length; i++) {
        ...
        const notificacao = new Notificacao_1.Notificacao(`notif_${notifData.usuario.id}_${i + 1}`, notifData.usuario, notifData.message, dataEnvio, lida);
        await repositories_1.notificacaoRepository.create(notificacao);
    }
}
function generateStockHistory(days, startPrice, volatility) {
    const history = [];
...
function generateBondHistory(days, startPrice, volatility) {
    const history = [];
    let currentPrice = startPrice;
    for (let i = 0; i < days; i++) {
...
function generateFundHistory(days, startPrice, volatility) {
    const history = [];
    let currentPrice = startPrice;
    for (let i = 0; i < days; i++) {
...
async function generateSeedReport() {
    const usuarios = await repositories_1.usuarioRepository.findAll();
    const investimentos = await repositories_1.investimentoRepository.findAll();
    const portfolios = await repositories_1.portfolioRepository.findAll();
    const favoritos = await repositories_1.favoritoRepository.findAll();
    const notificacoes = await repositories_1.notificacaoRepository.findAll();
    console.log('\n📊 === RELATÓRIO DO SEED ===\n');
